...
check_required_vars() {
    ...
    [ -z "$REDIS_PASSWORD" ] && export REDIS_PASSWORD="redispass"  # Значение по умолчанию для Redis
    [ -z "$REDIS_HOST" ] && export REDIS_HOST="redis"  # Значение по умолчанию для Redis host
    [ -z "$REDIS_PORT" ] && export REDIS_PORT="6379"  # Значение по умолчанию для Redis port
...
check_redis_connection() {
    local redis_password=${REDIS_PASSWORD:-redispass}
...
check_redis_caching() {
    local user_id=$1
    local redis_key="user:${user_id}"
    local result=$(docker exec redis redis-cli -a "$REDIS_PASSWORD" GET "$redis_key")
    
    echo "Checking Redis key: $redis_key" >> test_results.log
    echo "Redis response: $result" >> test_results.log
    
    if [ -z "$result" ] || [ "$result" = "null" ]; then
        print_warning "User data not found in Redis cache for key: $redis_key"
        return 1
    fi
    print_success "User data found in Redis cache"
    return 0
}
...
if ! check_redis_connection; then
    handle_error 1 "Cannot connect to Redis"
fi
...
print_step "Checking Redis caching"
sleep 2  # Даем время на кеширование
check_redis_caching "$USER_ID"
